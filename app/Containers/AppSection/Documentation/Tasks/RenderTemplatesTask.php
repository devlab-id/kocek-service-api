...
class RenderTemplatesTask extends AbstractTask
{
    ...
    public function __construct()
    {
        ...
        $this->replaceArray = [
            'api.domain.test' => config('kocek.api.url'),
            '{{rate-limit-expires}}' => config('kocek.api.throttle.expires'),
            '{{rate-limit-attempts}}' => config('kocek.api.throttle.attempts'),
            '{{access-token-expires-in}}' => $this->minutesToTimeDisplay(config('kocek.api.expires-in')),
            '{{access-token-expires-in-minutes}}' => config('kocek.api.expires-in'),
            '{{refresh-token-expires-in}}' => $this->minutesToTimeDisplay(config('kocek.api.refresh-expires-in')),
            '{{refresh-token-expires-in-minutes}}' => config('kocek.api.refresh-expires-in'),
