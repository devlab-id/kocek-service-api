...
trait RoutesLoaderTrait
{
    ...
    public function runRoutesAutoLoader(): void
    {
        ...
        $allContainerPaths = Kocek::getAllContainerPaths();
    ...
    private function getRateLimitMiddleware(): string|null
    {
        ...
        if (Config::get('kocek.api.throttle.enabled')) {
            RateLimiter::for('api', function (Request $request) {
                return Limit::perMinutes(Config::get('kocek.api.throttle.expires'), Config::get('kocek.api.throttle.attempts'))->by($request->user()?->id ?: $request->ip());
            });
    ...
    {
        return Config::get('kocek.api.url');
    }

    private function getApiVersionPrefix(SplFileInfo $file): string
    {
        return Config::get('kocek.api.prefix') . (Config::get('kocek.api.enable_version_prefix') ? $this->getRouteFileVersionFromFileName($file) : '');
    }
